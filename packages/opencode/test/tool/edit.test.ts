...
import { EditTool, replaceOutcome } from "../../src/tool/edit"
...
describe("tool.edit", () => {
  ...
  // oc railings (0323 JSON switch): replaceOutcome tier/rebase semantics,
  // changed-lines echo + files[]/paths/noop/stat metadata.
  describe("oc railings", () => {
    test("replaceOutcome exact match", () => {
      const r = replaceOutcome("a\nb\n", "a", "x")
      expect(r.text).toBe("x\nb\n")
      expect(r.tier).toBe("Simple")
      expect(r.rebased).toBe(0)
      expect(r.spanIndex).toBe(0)
    })

    test("replaceOutcome tolerance tier + 0301 rebase adopts file indent", () => {
      // The model copies OLD+NEW at 0 indent while the file region is 2-space
      // indented: LineTrimmed matches (whitespace-invisible), and the rebase
      // must land the NEW bytes at the FILE's indent, not the model's.
      const r = replaceOutcome("  const a = 1\n  const b = 2\n", "const a = 1\nconst b = 2", "const a = 9\nconst b = 9")
      expect(r.tier).toBe("LineTrimmed")
      expect(r.rebased).toBe(2)
      expect(r.text).toBe("  const a = 9\n  const b = 9\n")
    })

    test("replaceOutcome deliberate uniform re-indent preserved (not rebased)", () => {
      const r = replaceOutcome("const a = 1\nconst b = 2\n", "const a = 1\nconst b = 2", "  const a = 1\n  const b = 2")
      expect(r.rebased).toBe(0)
      expect(r.text).toBe("  const a = 1\n  const b = 2\n")
    })

    test("replaceOutcome multiple-match fails closed", () => {
      expect(() => replaceOutcome("a\na\n", "a", "b")).toThrow(/multiple matches/i)
    })

    it.instance("execute reports changed lines + files/paths/noop/stat metadata", () =>
      Effect.gen(function* () {
        const test = yield* TestInstance
        const filepath = path.join(test.directory, "file.txt")
        yield* put(filepath, "top = 0\nbottom = 0\n")
        const result = yield* run({ filePath: filepath, oldString: "top = 0", newString: "top = 1" })
        // Path prefix is worktree-relative (tmp/opencode-test-.../ in the
        // fixture) - assert the path-agnostic shape.
        expect(result.output).toContain("Changed lines: ")
        expect(result.output).toContain("file.txt: 1 (+1/-1)")
        expect(result.metadata.files).toHaveLength(1)
        expect(result.metadata.files[0]!.changed).toBe(true)
        expect(result.metadata.files[0]!.stat).toBeTruthy()
        expect(result.metadata.paths).toEqual([filepath])
        expect(result.metadata.noop).toBe(0)
        expect(yield* load(filepath)).toBe("top = 1\nbottom = 0\n")
      }))
  })
})
