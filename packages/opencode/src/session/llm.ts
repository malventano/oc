      ...
      Stream.scoped(
        Stream.unwrap(
          Effect.gen(function* () {
            ...
            // 300s (2026-09-14): 60s false-fired on GLM-5.3-Flash long-context
            // resume - a ~750K-token prefill takes ~60s+ of event-less stream
            // time (server-side prefill emits no SSE), so every resume attempt
            // was killed at the 60s mark and retried in a full-prefill loop.
            // Long prefill >> 60s is legitimate; 300s still catches dead
            // endpoints (BUG_RESPONSES_FAILURE class) without killing prefill.
              ...
              Effect.forever(
                Effect.gen(function* () {
                  ...
                  if (toolsInFlight === 0 && Date.now() - lastEvent >= IDLE_MS) {
                    yield* Effect.logWarning("llm idle watchdog fired", {
                      "session.id": input.sessionID,
                      idleMs: Date.now() - lastEvent,
                      threshold: IDLE_MS,
                      toolsInFlight,
                    })
                    yield* Deferred.fail(
