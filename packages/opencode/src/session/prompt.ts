      ...
      function* (sessionID: SessionID) {
        ...
        while (true) {
          ...
          if (
            ...
            // "Inject"-method compaction turn completed: the compaction user
            // message's final assistant is finished but not yet marked as the
            // summary. Finalize it (mark summary, retain tail, auto-continue)
            // instead of exiting the loop.
            const compactionUser = msgs.find((m) => m.info.id === lastUser.id)
            const compacting = compactionUser?.parts.some((p) => p.type === "compaction") ?? false
            const lastAssistantInfo = lastAssistantMsg?.info as SessionV1.Assistant | undefined
            if (compacting && lastAssistantInfo && !lastAssistantInfo.summary && !lastAssistantInfo.error) {
