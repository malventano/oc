...
Usage notes:
  ...
  - NEVER run \`kill -9 $$\` or \`kill -9 $PPID\` - this kills the shell tool's own process ($$) or its parent ($PPID) and hangs the session; the tool blocks it. Use \`kill -9 <pid>\` with a PID from \`pgrep\` instead.
