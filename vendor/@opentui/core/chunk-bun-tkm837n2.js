...
class CodeRenderable extends TextBufferRenderable {
  ...
  set content(value) {
    if (this._content !== value) {
      ...
      if (this._streaming && this._filetype && !this._drawUnstyledText) {
        // 0312: a streaming element that mounted before its first content
        // (the heredoc body slot at the transition) can consume the initial-
        // content window on an empty frame, so the raw-paint branch never
        // fires for the real content and the body depends only on the async
        // highlight - a superseded highlight leaves it blank while the props
        // gutter grows. Paint the FIRST real content raw (never painted yet),
        // then let the highlight color it - strictly better than blank.
  ...
  set streaming(value) {
    if (this._streaming !== value) {
      ...
      this._highlightsDirty = true;
      // 0311: at the streaming -> completed flip, guarantee the buffer is
      // paint-visible. The content is usually ALREADY this._content, so
      // set content is a no-op and the buffer only refreshes when the async
      // completion highlight lands - if that highlight is superseded, the
      // props-driven gutter counts all rows while the body stays unpainted
      // (the intermittent blank bash body). ensureVisible... paints raw now
      // if not already painted (and keeps the already-painting content
      // otherwise - no 0167 white flash).
      if (!value) {
  ...
  async startHighlight() {
    ...
    if (!filetype)
      return;
    const isInitialContent = this._streaming && !this._hadInitialContent;
    // 0312: do not consume the initial-content raw-paint window on an empty
    // frame (the heredoc body slot can highlight an empty content at the
    // transition) - leave it open so the first real content paints raw.
    if (isInitialContent && content.length > 0) {
    ...
    try {
      ...
      if (highlights.length > 0 || this._onChunks || this._baseHighlight) {
        ...
        {
          // 0322e: ALWAYS-ON apply-site probe (no env gate). cl = the content
          // snapshot this highlight colored; sl = the chunk text length
          // actually applied to the buffer. sl < cl = the applied styled text
          // lost content (stale/missing chunks) - the live symptom path for
          // the reasoning/answer mid-line cuts. Pushed to a global array; the
          // TUI part-trace flushes + logs it (never stderr).
          let __sl = 0;
          for (const __ch of chunks || []) __sl += (__ch.text && __ch.text.length) || 0;
          ((globalThis).__ocPartApplyEvents ??= []).push({ cl: content.length, sl: __sl, cc: content.slice(0, 32) });
        }
