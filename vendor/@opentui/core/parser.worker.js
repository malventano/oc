...
class ParserWorker {
  ...
  // 0326 (0322s): per-ELEMENT parser. 0322n's Mode A was concurrent use of the
  // ONE shared per-filetype parser (not reentrant - _ts_tree_delete crash +
  // poison). Giving each streaming element its OWN parser removes the sharing
  // entirely - different elements highlight CONCURRENTLY on distinct parsers
  // (no queue-storm - the global chain's 70-85ms worst case on many elements)
  // while same-element highlights stay ordered on the element's own chain.
  // Free the parser when the element's one-shot cache entry is evicted/dropped.
  async getElementParser(filetype, cacheKey) {
    const canonicalFiletype = this.resolveCanonicalFiletype(filetype);
    if (this.elementParsers?.has(cacheKey)) {
      return this.elementParsers.get(cacheKey);
    }
    const state = await this.createReusableParser(canonicalFiletype);
    if (state) {
      if (!this.elementParsers) this.elementParsers = new Map();
      this.elementParsers.set(cacheKey, state);
    }
    return state;
  }
  dropElementParser(cacheKey) {
    if (this.elementParsers?.has(cacheKey)) {
      const state = this.elementParsers.get(cacheKey);
      if (state?.parser) state.parser.delete();
      this.elementParsers.delete(cacheKey);
    }
  }
  ...
  async handleOneShotHighlight(content, filetype, messageId, cacheKey) {
    const total0 = performance.now();
    const reusableState = cacheKey ? await this.getElementParser(filetype, cacheKey) : await this.getReusableParser(filetype);
    this.hlDbg(cacheKey, "parser", { len: (content || "").length });
    if (!reusableState) {
      postWorkerMessage({
...
` : content;
    // 0326 (0322r): the one-shot cache is keyed PER ELEMENT (cacheKey). 0322o's
    // Mode B came from the single-slot filetype key handing one element's tree
    // to a different element sharing a prefix - per-element keys make
    // append-only incremental reuse SAFE again (O(delta) parse + scoped
    // injections = the 0308 flat refresh). Elements without a cacheKey get no
    // incremental (fresh parse, correct - the 0322o behavior for those).
    let cached;
    let cacheRes = { key: !!cacheKey, found: false, mode: "nocache" };
    if (cacheKey && this.oneShotParseCache) {
      const entry = this.oneShotParseCache.get(cacheKey);
      if (entry) {
        if (entry.filetype === filetype) {
          cached = entry;
          cacheRes = { key: true, found: true, mode: "same" };
        } else {
          if (entry.tree) entry.tree.delete();
          this.oneShotParseCache.delete(cacheKey);
          this.dropElementParser(cacheKey);
          cacheRes = { key: true, found: false, mode: "filetype" };
        }
      } else {
        cacheRes = { key: true, found: false, mode: "miss" };
      }
    }
    let tree;
    ...
    if (cached && parseContent.length > cached.content.length && parseContent.startsWith(cached.content)) {
      ...
      this.hlDbg(cacheKey, "parse-inc", { len: parseContent.length });
      ...
      if (cached) {
        cached.tree.delete();
        if (cacheKey) this.oneShotParseCache?.delete(cacheKey);
      }
      this.hlDbg(cacheKey, "parse-full", { len: parseContent.length });
      const t0 = performance.now();
    ...
    try {
      ...
      let matches;
      this.hlDbg(cacheKey, "query", { len: parseContent.length });
      ...
      if (queries.injections) {
        ...
        this.hlDbg(cacheKey, "inject", { len: parseContent.length });
      ...
      this.hlDbg(cacheKey, "simple", { len: parseContent.length });
      t0 = performance.now();
      const fresh = this.getSimpleHighlights([...matches, ...injectionMatches], injectionRanges);
      // 0322t REVERTED (2026-09-08): the delta protocol regressed restored-
      // content rendering (short/empty buffers on rehydrated reasoning parts)
      // and re-triggered the 0322g-class worker stall on undo+submit (no JS
      // error - the hang class; needs the hang root-caused first). Back to the
      // full-list 0309 key-dedupe merge (O(n) - the known residual slope).
      let highlights;
      if (incremental && cached && cached.highlights) {
        // 0326 (0322w): corrected 3-way merge. The key-dedupe union (0309) held
        // BOTH the cached and fresh versions of the same region's spans - their
        // boundaries shift slightly between incremental steps, so the paint
        // toggled (the streaming color jitter). Take the PREFIX from the cached
        // list verbatim (stable - the applied version, never re-derived from the
        // incremental tree) and the CHANGE REGION from the fresh parse (the only
        // part the append changed - newly parsed = correct). Key-dedupe ONLY the
        // region so overlapping stale/new spans collapse to one version. Also
        // O(delta) instead of O(n) - the 0309 merge was the climb.
        const prefix = [];
        const region = [];
        for (const h of cached.highlights) {
          if (h[1] < queryStart) prefix.push(h);
          else region.push(h);
        }
        ...
        highlights = prefix.concat([...merged.values()].sort((a, b) => a[0] - b[0]));
        // 0326 (0322x): merge-stats diagnostic - prefix/region/fresh sizes + span
        // extent, so the streaming color-drop trace shows what the merge retained.
        try {
          let mn = 0, mx = 0;
          for (const h of highlights) { if (h[1] > mx) mx = h[1]; }
          this.hlDbg(cacheKey, "merge", { cl: parseContent.length, n: highlights.length, prefix: prefix.length, region: region.length, fresh: fresh.length, mn: highlights.length ? Math.min(...highlights.map((h) => h[0])) : 0, mx });
        } catch {}
      } else {
        highlights = fresh;
      }
      ...
      // 0326 (0322t DELTA - the 2nd-round 8ms, element-side): the incremental
      // append sends ONLY the change-region captures (end >= queryStart) as
      // `delta` + `deltaMode` + `changedStart`; the CodeRenderable merges them
      // into its _lastHighlights (key-dedupe). Cold/full responses send the
      // authoritative full `highlights`. (The 0322t revert was for the RESTORED-
      // content regression + the delta-triggered stall - the client merge stays
      // element-side where the baseline is element-scoped.)
      let delta = null;
      let sendHighlights = highlights;
      let deltaMode = false;
      let changedStart = 0;
      if (incremental && cached?.highlights?.length) {
        deltaMode = true;
        changedStart = queryStart;
        delta = [];
        for (const h of highlights) {
          if (h[1] >= queryStart) delta.push(h);
        }
        sendHighlights = [];
      }
      postWorkerMessage({
        ...
        messageId,
        hasParser: true,
        highlights: sendHighlights,
        incremental,
        timings,
        deltaMode,
        delta,
        changedStart,
        d: { ...cacheRes, len: parseContent.length, cachedLen: cached?.content.length ?? 0, pfx: cached ? parseContent.startsWith(cached.content) : undefined }
      });
      ...
      if (cacheKey) {
        if (!this.oneShotParseCache) this.oneShotParseCache = new Map();
        this.oneShotParseCache.set(cacheKey, { filetype, content: parseContent, tree, highlights: fresh });
        while (this.oneShotParseCache.size > 12) {
          const oldestKey = this.oneShotParseCache.keys().next().value;
          const oldest = this.oneShotParseCache.get(oldestKey);
          if (oldest?.tree) oldest.tree.delete();
          this.oneShotParseCache.delete(oldestKey);
          this.dropElementParser(oldestKey);
        }
      }
    } finally {
  ...
  async clearCache() {
    ...
    try {
      ...
      if (this.elementParsers) {
        for (const state of this.elementParsers.values()) {
          if (state?.parser) state.parser.delete();
        }
        this.elementParsers = undefined;
      }
      this._elemChains = undefined;
      if (this.oneShotParseCache) {
        for (const entry of this.oneShotParseCache.values()) {
          if (entry?.tree) entry.tree.delete();
        }
    ...
    try {
      switch (message.type) {
        ...
        case "ONESHOT_HIGHLIGHT":
          // 0322n: the shared tree-sitter Parser + oneShotParseCache are NOT
          // reentrant-safe: multiple async highlight handlers overlap (the
          // reasoning + answer + tool-stream code elements all highlight on the
          // ONE reusable parser), concurrent parse()/delete() corrupted the
          // tree ownership and crashed with `RuntimeError: Out of bounds
          // memory access` in _ts_tree_delete (the LIVE root-cause error,
          // 2026-09-08), poisoning the parser for the rest of the session
          // (the cumulative freeze - needs enough overlapping highlights to
          // collide; undo/redo + consecutive prompts collide instantly).
          // Serialize every highlight so the parser is used by exactly one
          // handler at a time.
          // 0326 (0322s): PER-ELEMENT chain + parser. Same-element highlights
          // wait on the element's own chain (one parser is not reentrant - the
          // element fires a highlight per render). DIFFERENT elements run on
          // their own parsers CONCURRENTLY - no global queue-storm (the
          // _hlChain's 70-85ms worst case with ~20 elements re-highlighting,
          // live 2026-09-08). CacheKey-less callers keep the shared per-
          // filetype parser + the global chain (0322n behavior, safe).
          // Poison-proof: a rejecting handler must not skip every later .then
          // (the 0322g-class "stops answering" - seen live when the incremental
          // reintroduced error sources). The error is reported (the client
          // falls back to raw text) and the chain lives on.
          const run = (handler) =>
            Promise.resolve()
              .then(handler)
              .catch((error) => {
          ...
          if (message.cacheKey) {
            if (!worker._elemChains) worker._elemChains = new Map();
            const prev = worker._elemChains.get(message.cacheKey) ?? Promise.resolve();
            const next = prev.then(() => run(() => worker.handleOneShotHighlight(message.content, message.filetype, message.messageId, message.cacheKey)));
            worker._elemChains.set(message.cacheKey, next);
            try {
              await next;
            } finally {
              if (worker._elemChains.get(message.cacheKey) === next) worker._elemChains.delete(message.cacheKey);
            }
          } else {
            worker._hlChain = (worker._hlChain ?? Promise.resolve())
              .then(() => run(() => worker.handleOneShotHighlight(message.content, message.filetype, message.messageId, undefined)))
              .catch(() => {});
            await worker._hlChain;
          }
          break;
