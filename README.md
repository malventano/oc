...
- **`bun install` at the repo root is REQUIRED**: the build bundles the **vendored** `@opentui/core` + `@opentui/solid` (`vendor/@opentui/...`, wired as `file:` workspace deps - our render fixes live there as committed source: 0196 drawUnstyledText diff setter / no white-flash, 0201 scroll height-delta anchor, the 0322n/o highlight serialization, the element-side delta), plus the other patched deps under `patchedDependencies` (solid-js, effect, ai-sdk providers, tanstack, etc.). The opentui files are vendored (not patched) so a fresh clone builds byte-identical output with no patch-application fragility. A build run with `--skip-install` WITHOUT a prior `bun install` would bundle STOCK opentui and silently lose those fixes. `--skip-install` is only a speedup for the *artifact* step; it skips the extra cross-platform `bun install @opentui/core@...` in build.ts, not the root `bun install` that links the vendor tree.
- On an upstream opentui bump: re-copy the stock package into `vendor/@opentui/{core,solid}` and re-apply our fixes (grep the vendored source for the oc markers; the old patch hunks in git history are the porting guide).
